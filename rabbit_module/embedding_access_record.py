...
def get_col_spotify(inputBatch):
    x_both, y = inputBatch
    x_cat = list(x_both.values())[0:15]
    x_int = torch.cat(list(x_both.values())[15:20],1)
    length = y.shape[0]
    y = y.reshape(length,1)
    x_cat = torch.stack(x_cat)
    return y, x_cat, x_int
...
if __name__ == "__main__":
    ...
    if dataset == "kaggle":
        ...
        CONTINUOUS_COLUMNS = [
            "session_position", "session_length", "hist_user_behavior_n_seekfwd", 
            "hist_user_behavior_n_seekback", "hour_of_day"
        ]
        CATEGORICAL_COLUMNS = [
            "session_id", "track_id_clean", "skip_1", "skip_2", "skip_3", "not_skipped",
    ...
    if dataset == "kaggle":
        ...
        for i in range(length):
            torch.save(access_list[i], '/workspace/SC_artifacts_eval/Access_Index/spotify/access_record/access_'+str(i)+'.pt')